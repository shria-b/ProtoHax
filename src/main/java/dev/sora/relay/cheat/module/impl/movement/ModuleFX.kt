...
class ModuleFX : CheatModule("FX", CheatCategory.MOVEMENT) {
	...
	private var strafeSpeedValue by intValue("StrafeSpeed", 20, 10..526).visible { modeValue == Mode.STRAFE }
			...
			if(modeValue == Mode.SPEED){
				if (player.onGround || player.motionY == 0f || (player.motionY > player.prevMotionY && player.motionY < 0f)) {
					session.netSession.inboundPacket(SetEntityMotionPacket().apply {
						...
						motion = Vector3f.from((-sin(direction) * speedValue), -0.02f, (cos(direction) * speedValue))
					})
				}else {
					session.netSession.inboundPacket(SetEntityMotionPacket().apply {
						runtimeEntityId = session.player.runtimeEntityId
